...
アカウントで、**Windows のユーザー名やパスワードとは無関係**。好きに決めてよい。

```
Enter new UNIX username: bambu
New password:
Retype new password:
```

- **ユーザー名**は半角小文字の英字で始める（例: `bambu`、`ishihara`）。
  大文字・空白・日本語は使えない。Windows のユーザー名が日本語や空白入りでも、
  ここは関係ないので短い英字名にしておくとこの先が楽。
- **パスワードは入力しても画面に何も表示されない**（`*` すら出ない）。
  故障ではないので、そのまま打って Enter を押す。同じものをもう一度求められる。
- このパスワードは Linux 側で管理者操作（`sudo`）をするときにだけ使う。
  このツールを使うだけなら出番はないが、忘れると再設定が面倒なので控えておく。

`bambu@PC名:~$` のような表示が出れば完了。
